...
function render(s){
 ...
 ["chainName","displayTitle","clinicName","clinicDisplayName","homeEyebrow","specialty","welcomeText","comfortText"].forEach(id=>syncValue(id,s.settings?.[id]||""));
 syncValue("qrClinic",s.settings?.clinicName||"عيادة د. طاهر");syncValue("qrDoctor",s.settings?.doctorName||"");
...
$("save").onclick=()=>chairAPI.saveSettings({
  doctorName:$("doctor").value,
...
}).then(()=>note({message:"تم حفظ الإعدادات وتحديث الشاشة",type:"success"}));
