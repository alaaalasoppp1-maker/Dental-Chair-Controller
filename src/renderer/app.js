...
async function loadPatientArchive(category=archiveCategory){
  ...
  try{
    ...
    data.items.forEach(item=>{const button=document.createElement("button");button.type="button";button.className=`archive-item ${archiveSelected?.path===item.path?"active":""}`;button.innerHTML=`<span class="archive-thumb">${item.kind==="image"?"🖼":"📄"}</span><b>${escapeHtml(item.name)}</b><small>${escapeHtml(item.category)} · ${new Date(item.modifiedAt).toLocaleDateString("ar")}</small>`;button.onclick=()=>selectArchiveItem(item,button);grid.appendChild(button);if(item.kind==="image")chairAPI.previewArchive(item.path).then(preview=>{const thumb=button.querySelector(".archive-thumb");if(thumb&&preview.dataUrl)thumb.innerHTML=`<img src="${preview.dataUrl}" alt="">`}).catch(()=>{})});
...
async function selectArchiveItem(item,button){
  archiveSelected=item;document.querySelectorAll(".archive-item").forEach(el=>el.classList.remove("active"));button?.classList.add("active");const image=$("archivePreviewImage"),record=$("archivePreviewRecord"),empty=$("archivePreviewEmpty");image.style.display="none";record.style.display="none";empty.style.display="block";$("archivePreviewName").textContent=item.name;
  try{const preview=await chairAPI.previewArchive(item.path);if(preview.kind==="image"){image.src=preview.dataUrl;image.style.display="block";empty.style.display="none"}else if(preview.kind==="record"){record.textContent=JSON.stringify(preview.value,null,2);record.style.display="block";empty.style.display="none"}}
  catch(error){note({message:error.message||"تعذر فتح المعاينة",type:"error"})}
}
function openPatientArchive(){archiveSelected=null;archiveCompare=[];$("patientArchiveModal").classList.add("open");$("patientArchiveModal").setAttribute("aria-hidden","false");$("archiveComparePanel").hidden=true;loadPatientArchive("all")}
