...
async function openClinicalPlan(plan){
  ...
  try{const data=await chairAPI.getClinicalPlanDetail(plan.planId);clinicalPlanData=data;$("clinicalPlanTitle").textContent=data.title||plan.title||"تقرير الخطة";$("clinicalPlanSummary").textContent=`${plan.progress||data.report?.progress||0}% · ${plan.sessions} جلسة · ${plan.events} حدث سريري`;renderClinicalPlanReport(data);renderClinicalTimeline(data.events||[]);showClinicalReport()}catch(error){note({message:error.message||"تعذر قراءة تفاصيل الخطة",type:"error"})}
}
...
function renderClinicalPlanReport(data){const report=data.report||{};$("reportReachedStage").textContent=report.reachedStage||"لم تبدأ بعد";$("reportDuration").textContent=treatmentDuration(report.totalTreatmentMs);$("reportAnesthesia").textContent=`${Number(report.anestheticQuantity||0)} ${report.anestheticUnit||"أمبولة"}`;$("reportRestoration").textContent=report.finalRestoration||"غير مسجل";$("reportNotes").innerHTML=(report.notes||[]).length?(report.notes||[]).map(item=>`<p><time>${clinicalDate(item.at)} · ${clinicalTime(item.at)}</time>${escapeHtml(item.text)}</p>`).join(""):"لا توجد ملاحظات مسجلة."}
...
$("clinicalShowOrdinary").onclick=()=>showClinicalMedia("ordinary");$("clinicalShowXrays").onclick=()=>showClinicalMedia("xray");$("clinicalShowDetails").onclick=showClinicalDetails;$("clinicalStopDisplay").onclick=()=>chairAPI.hide();document.querySelectorAll("[data-clinical-report]").forEach(button=>button.onclick=showClinicalReport);
