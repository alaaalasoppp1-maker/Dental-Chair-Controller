...
const TEETH=[...PERMANENT_TEETH,...PRIMARY_TEETH];
...
function renderToothChart(host,selected,onClick,{stageId=""}={}){
  ...
  const makeArch=(title,items,kind)=>{
    const section=document.createElement("section");section.className=`dental-arch ${kind}`;section.innerHTML=`<small>${title}</small><div class="dental-arch-row"></div>`;
    const row=section.querySelector(".dental-arch-row");
    items.forEach((tooth,index)=>{const colors=[...new Set(annotations.filter(item=>Number(item.tooth)===tooth&&(!stageId||annotationStage(item)?.id===stageId)).map(item=>labelForAnnotation(item).color))],button=document.createElement("button");button.type="button";button.innerHTML=`<span>${tooth}</span>`;button.setAttribute("aria-label",`السن ${tooth}`);button.dataset.tooth=tooth;button.style.setProperty("--arch-index",index);button.classList.toggle("selected",selectedSet.has(tooth));button.classList.toggle("linked",Boolean(colors.length));if(colors.length)button.style.setProperty("--tooth-color",colors[0]);button.onclick=()=>onClick(tooth);row.appendChild(button);});
    host.appendChild(section);
  };
  makeArch("الأسنان الدائمة · الفك العلوي",PERMANENT_TEETH.slice(0,16),"upper permanent");
  makeArch("الأسنان اللبنية · الفك العلوي",PRIMARY_TEETH.slice(0,10),"upper primary");
  makeArch("الأسنان اللبنية · الفك السفلي",PRIMARY_TEETH.slice(10),"lower primary");
  makeArch("الأسنان الدائمة · الفك السفلي",PERMANENT_TEETH.slice(16),"lower permanent");
}
...
function newStage(title="مرحلة علاج جديدة",color="#2f8fe9",autoCreated=false){const priority=title==="عاجل"?"عاجل":title==="مهم"?"مهم":title==="بارد"?"بارد":"للمراقبة";return{id:uid("stage"),title,description:"",priority,sessions:1,duration:"",cost:0,color,manualTeeth:[],excludedTeeth:[],illustrationId:"",illustrationMode:"none",illustrationPath:"",illustrationDataUrl:"",illustrationName:"",backgroundPath:"",autoCreated};}
...
function renderIllustrations(){
  ...
  const none=document.createElement("button");none.type="button";none.className=`illustration-choice none ${!stage.illustrationId?"active":""}`;none.innerHTML="<span>دون صورة</span>";none.onclick=()=>{clearStageIllustration(stage);renderIllustrations();renderPresentation();};host.appendChild(none);
  illustrations.forEach(item=>{const button=document.createElement("button");button.type="button";button.className=`illustration-choice ${stage.illustrationId===item.id?"active":""}`;button.innerHTML=`<img src="${item.dataUrl}" alt=""><span>${escapeHtml(item.name)}</span>`;button.onclick=()=>{stage.illustrationId=item.id;stage.illustrationMode="selected";stage.illustrationPath=item.path||"";stage.illustrationDataUrl=item.dataUrl||"";stage.illustrationName=item.name||"";renderIllustrations();renderPresentation();};host.appendChild(button);});
}
function renderPlan(){
...
function clearStageIllustration(stage=currentStage()){if(!stage)return;stage.illustrationId="";stage.illustrationMode="none";stage.illustrationPath="";stage.illustrationDataUrl="";stage.illustrationName="";stage.imagePath="";}
function stagePayload(stage){
  const illustration=stage.illustrationMode==="none"?null:illustrationById(stage.illustrationId);
  const hasIllustration=stage.illustrationMode!=="none"&&Boolean(illustration||stage.illustrationPath||stage.illustrationDataUrl);
  return{...clone(stage),teeth:stageTeeth(stage),labelIds:labelsForStage(stage).map(label=>label.id),illustrationMode:hasIllustration?"selected":"none",illustrationName:hasIllustration?(illustration?.name||stage.illustrationName||""):"",illustrationPath:hasIllustration?(illustration?.path||stage.illustrationPath||""):"",illustrationDataUrl:hasIllustration?(illustration?.dataUrl||stage.illustrationDataUrl||""):"",imagePath:""};
}
function planPayload(){
  storeStage();
  const total=totals(),durations=stages.map(stage=>String(stage.duration||"").trim()).filter(Boolean);
  return{id:planId||undefined,title:$("planTitle").value.trim()||"خطة العلاج المقترحة",currency:$("planCurrency").value,panoramaPath:planPanoramaPath,panoramaFileName:source?.name||"",sourcePath:source?.path||"",sourceName:source?.name||"",sourceWidth:sourceImage?.naturalWidth||0,sourceHeight:sourceImage?.naturalHeight||0,panoramaAspectRatio:sourceImage?.naturalWidth&&sourceImage?.naturalHeight?sourceImage.naturalWidth/sourceImage.naturalHeight:0,closingNote:$("planClosing").value.trim(),labels:clone(labels),annotations:clone(annotations),annotatedImageDataUrl:sourceImage?annotatedDataUrl(""):"",stages:stages.map(stagePayload),totalCost:total.cost,totalSessions:total.sessions,totalDuration:durations.join(" + "),doctorName:appState.patient?.doctorName||""};
}
async function loadPlan(){
  ...
  try{
    ...
    if(plan.sourceDataUrl)await setSource({path:plan.sourcePath||plan.panoramaPath,name:plan.panoramaFileName||plan.sourceName||"صورة الخطة",dataUrl:plan.sourceDataUrl},{preservePlan:true});else if(plan.panoramaMissing)notify(`صورة البانوراما المرتبطة بالخطة غير موجودة: ${plan.panoramaFileName||"ملف غير معروف"}`);
    ...
    stages=(plan.stages||[]).map(stage=>({...stage,illustrationMode:(stage.illustrationMode==="selected"||(stage.illustrationId||stage.illustrationPath||stage.illustrationDataUrl))?"selected":"none",manualTeeth:(stage.manualTeeth||stage.teeth||[]).map(Number),excludedTeeth:(stage.excludedTeeth||[]).map(Number)}));activeStage=0;presentationFocusStageId="";
...
function validatePlan(){
  const warnings=[];
  if(!sourceImage)warnings.push("لا توجد صورة بانوراما مرتبطة بالخطة");
  if(!source?.path)warnings.push("مسار صورة البانوراما غير محفوظ");
  stages.forEach((stage,index)=>{if(!String(stage.title||"").trim())warnings.push(`المرحلة ${index+1} بلا عنوان`);if(!Number(stage.sessions||0))warnings.push(`المرحلة ${index+1} بلا عدد جلسات`);});
  return warnings;
}
async function savePlan(showAll=false){
  try{
    const warnings=validatePlan();
    if(warnings.length&&!confirm(`توجد ملاحظات على الخطة:

${warnings.join("\n")}

هل تريد الحفظ رغم ذلك؟`))return null;
...
async function openPreviousPlans(){
  ...
  list.innerHTML=plans.map(plan=>`<div class="previous-plan-row"><button type="button" data-open-plan="${escapeHtml(plan.id)}"><span><b>${escapeHtml(plan.title||"خطة علاج")}</b><small>${plan.stagesCount||0} مراحل · ${Number(plan.totalCost||0).toLocaleString("en")} ${escapeHtml(plan.currency||"")}</small></span><em>فتح ←</em></button><button type="button" class="danger previous-plan-delete" data-delete-plan="${escapeHtml(plan.id)}" data-plan-title="${escapeHtml(plan.title||"خطة علاج")}">حذف</button></div>`).join("");
  ...
  list.querySelectorAll("[data-delete-plan]").forEach(button=>button.onclick=async()=>{if(!confirm(`حذف خطة «${button.dataset.planTitle}» نهائياً؟`))return;try{await chairAPI.deletePlan(button.dataset.deletePlan);await openPreviousPlans();}catch(error){notify(error);}});
}
...
function syncStageTextFields(){
  storeStage();
  renderStageList();
  const total=totals();
  $("planTotal").textContent=`${total.cost.toLocaleString("en")} ${$("planCurrency").value}`;
  $("planSessionsTotal").textContent=`${total.sessions} جلسة`;
  renderPresentation();
}
[$("stageTitle"),$("stageDescription"),$("stagePriority"),$("stageSessions"),$("stageDuration"),$("stageCost"),$("stageColor")].forEach(element=>{
  ["input","change","blur"].forEach(eventName=>element.addEventListener(eventName,syncStageTextFields));
});
[$("planTitle"),$("planCurrency"),$("planClosing")].forEach(element=>{
  ["input","change","blur"].forEach(eventName=>element.addEventListener(eventName,()=>renderPresentation()));
});
$("importStageIllustration").onclick=async()=>{try{const item=await chairAPI.importStageIllustration();if(item){await refreshIllustrations();const stage=currentStage();if(stage){stage.illustrationId=item.id;stage.illustrationMode="selected";stage.illustrationPath=item.path||"";stage.illustrationDataUrl=item.dataUrl||"";stage.illustrationName=item.name||"";}renderIllustrations();renderPresentation();}}catch(error){notify(error);}};
$("clearStageIllustration").onclick=()=>{const stage=currentStage();if(!stage)return;clearStageIllustration(stage);renderIllustrations();renderPresentation();};
