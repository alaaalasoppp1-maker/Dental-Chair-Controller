...
const DEFAULT_LABELS=[
  {id:"label-urgent",name:"عاجل",color:"#ef496f",stageId:""},
  {id:"label-cold",name:"بارد",color:"#36c98f",stageId:""},
  {id:"label-priority",name:"أولوية",color:"#ff9f43",stageId:""},
  {id:"label-watch",name:"مراقبة",color:"#2fa8ff",stageId:""}
];
...
let visualRevision=0,visualCache=new Map();
...
async function refreshPlans(){
  ...
  if(!patientReady())return;
  try{(await chairAPI.listPlans()).forEach(plan=>select.add(new Option(`${plan.title} · ${plan.stagesCount} مراحل · ${plan.totalCost||0} ${plan.currency||""}`,plan.id)));}catch{}
}
...
function renderAnnotationList(){
  ...
  annotations.forEach((annotation,index)=>{const label=labelForAnnotation(annotation),stage=annotationStage(annotation),button=document.createElement("button");button.type="button";button.className=`annotation-row ${annotation.id===selectedAnnotationId?"active":""}`;button.innerHTML=`<i style="background:${escapeHtml(label.color)}"></i><span><b>${index+1}. ${escapeHtml(label.name)}</b><small>${annotation.tooth?`السن ${annotation.tooth}`:"دون سن"} · ${escapeHtml(stage?.title||"غير مرتبطة")}</small></span><em>⌖</em>`;button.onclick=()=>{selectedAnnotationId=annotation.id;activeLabelId=annotation.labelId;activeTooth=annotation.tooth||"";setDrawTool("select");renderAllStudio();};host.appendChild(button);});
  if(!annotations.length)host.innerHTML='<p class="studio-help">لا توجد رسومات بعد.</p>';
}

function newStage(title="مرحلة علاج جديدة",color="#19b8f2",autoCreated=false){return{id:uid("stage"),title,description:"",points:[],priority:"متوسطة",prognosis:"جيد",sessions:1,duration:"",cost:0,color,manualTeeth:[],excludedTeeth:[],illustrationId:"",backgroundPath:"",autoCreated};}
...
function storeStage(){
  ...
  stage.points=$("stagePoints").value.split("\n").map(value=>value.trim()).filter(Boolean);
  stage.priority=$("stagePriority").value;
  stage.prognosis=$("stagePrognosis").value;
...
function setStageFormEnabled(enabled){["stageTitle","stageDescription","stagePoints","stagePriority","stagePrognosis","stageSessions","stageDuration","stageCost","stageColor","stageBackgroundPath","chooseStageBackground","clearStageBackground"].forEach(id=>$(id).disabled=!enabled);}
function loadStageForm(){
  const stage=currentStage();setStageFormEnabled(Boolean(stage));
  if(!stage){$("stageTitle").value="";$("stageDescription").value="";$("stagePoints").value="";$("stageBackgroundPath").value="";return;}
  $("stageTitle").value=stage.title||"";
  $("stageDescription").value=stage.description||"";
  $("stagePoints").value=(stage.points||[]).join("\n");
  $("stagePriority").value=stage.priority||"متوسطة";
  $("stagePrognosis").value=stage.prognosis||"جيد";
...
$("openStudioPlan").onclick=()=>openStudio("plan");$("openStudioAnalysis").onclick=()=>openStudio("draw");$("closeClinicalStudio").onclick=closeStudio;
...
$("useCurrentXray").onclick=()=>useCurrentSource(true);$("useSelectedPanorama").onclick=async()=>{const path=$("patientPanoramaList").value;if(!path)return notify("اختر بانوراما");await chairAPI.showPanorama(path);await useCurrentSource(true);};
...
$("loadPlan").onclick=loadPlan;$("savePlan").onclick=()=>savePlan(false);$("presentPlan").onclick=()=>savePlan(true);$("newPlan").onclick=()=>{if(!annotations.length||confirm("بدء خطة جديدة ومسح الرسومات الحالية؟"))newPlan();};
...
chairAPI.onOpenClinicalStudio?.(step=>openStudio(step||"draw"));chairAPI.onState(state=>{appState=state||{};updatePatientLabel();});chairAPI.getState().then(state=>{appState=state||{};updatePatientLabel();});
