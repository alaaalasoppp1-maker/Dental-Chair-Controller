...
contextBridge.exposeInMainWorld("chairAPI",{
  ...
  getClinicalPlanDetail:request=>ipcRenderer.invoke('archive:clinical-plan-detail',request),
  queuePatientCloudArchive:()=>ipcRenderer.invoke('cloud:queue-patient'),
  toggleClinic:open=>ipcRenderer.invoke('clinic-pane:toggle',open),
  clinicAction:action=>ipcRenderer.invoke('clinic-pane:action',action),
  onClinicState:cb=>ipcRenderer.on('clinic-pane:state',(_e,state)=>cb(state)),
  onOpenPlanDetails:cb=>ipcRenderer.on('ui:open-plan-details',(_e,request)=>cb(request)),
