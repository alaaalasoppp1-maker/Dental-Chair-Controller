...
class PatientArchive{
  ...
  listClinicalPlans(){
    ...
    for(const entry of fs.readdirSync(root,{withFileTypes:true}).filter(item=>item.isDirectory()&&!item.name.startsWith("."))){
      ...
      if(!closure&&!sessions.length&&!progress&&!resume&&!individualEvents.length&&!previous.planId)continue;
  ...
  clinicalPlanDetail(planId){
    ...
    for(const file of walkFiles(dir)){
      ...
      if(!IMAGE_EXTENSIONS.has(ext))continue;item.category=radiograph?"xray":"ordinary";images.push(item);
    ...
    const recordedDuration=timeline.filter(item=>item.kind==="session_finished"||item.kind==="plan_completed").reduce((sum,item)=>sum+(Number(item.details?.durationMs)||0),0),resumeDuration=Number(resume?.resumeState?.totalTreatmentMs||0),lastEvent=timeline.at(-1)||{},report={progress:Math.max(Number(resume.progress||0),Number(progressRecord.progress||0)),reachedStage:String(resume.reachedStage||lastEvent.screenTitle||lastEvent.stage||"لم تبدأ بعد"),totalTreatmentMs:Math.max(recordedDuration,resumeDuration),anestheticQuantity:anesthesia,anestheticUnit:"أمبولة",finalRestoration:finalRestoration||"غير مسجل",notes,status:closure?"closed":Number(resume.progress||0)>=100?"ready_to_close":"active"};
