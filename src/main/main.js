...
function treatmentList(){return Array.isArray(settings?.get("treatments"))?settings.get("treatments").map(x=>({...x})):[];}
...
function treatmentById(id){return treatmentList().find(x=>String(x.id)===String(id))||null;}
...
function protocolPayload(raw){
  try{const url=new URL(String(raw||""));return JSON.parse(decodeBase64UrlUtf8(url.searchParams.get("data")||"")||"{}");}catch{return{};}
}
...
function handleProtocolUrl(raw){
  try{
    ...
    const payload=protocolPayload(raw);
    if(payload.action==="clear_patient"){ state.patient=archive?.clear()||{selected:false}; server?.setCurrentState?.({type:"home",sessionId:""}); emit(); return true; }
    if(payload.action==="select_patient"){
      state.patient=archive?.select(payload)||{selected:false}; server?.setSession?.(state.patient.sessionId||payload.sessionId||""); emit();return true;
    }
    if(payload.action==="show_patient"){
      ...
      const gender=["male","female"].includes(String(payload.gender))?String(payload.gender):"";
      const greeting=gender==="male"?`أهلًا بك سيد ${displayName}`:gender==="female"?`أهلًا بك سيدة ${displayName}`:`أهلًا بك ${displayName}`;
      server?.setSession?.(state.patient?.sessionId||payload.sessionId||"");
      server?.send({type:"patient",displayName,fullName,gender,greeting,doctorName,clinicName,sessionId:state.patient?.sessionId||payload.sessionId||""},{important:true});
...
function createWindow(){
  const silentStart=Boolean(pendingProtocolUrl&&protocolPayload(pendingProtocolUrl).action==="select_patient");
...
function ipc(){
  ...
  ipcMain.handle("archive:clear-patient",()=>archive.clear());
  ...
  ipcMain.handle("panorama:open-folder",()=>shell.openPath(archive.panoramaFolder()));
  ipcMain.handle("panorama:show",(_e,file)=>{
    const allowed=archive.listPanoramas().find(x=>x.path===file);if(!allowed)throw new Error("الصورة ليست ضمن مجلد بانوراما المريض");showFile(allowed.path,"image",true);return true;
  });
  ipcMain.handle("plan:choose-image",async(_e,kind)=>{
    const result=await dialog.showOpenDialog(win,{title:kind==="background"?"اختر خلفية المرحلة":"اختر الصورة التعليمية",properties:["openFile"],filters:[{name:"Images",extensions:["png","jpg","jpeg","webp","bmp"]}]});
    return result.canceled?null:result.filePaths[0];
  });
  ipcMain.handle("plan:save",(_e,plan)=>archive.savePlan(plan));
  ipcMain.handle("plan:export-composite",(_e,dataUrl,name)=>archive.exportComposite(dataUrl,name));
  ipcMain.handle("plan:list",()=>archive.listPlans());
  ipcMain.handle("plan:load",(_e,id)=>archive.loadPlan(id));
  ipcMain.handle("plan:show",(_e,plan)=>{
    const saved=plan?.id?archive.loadPlan(plan.id):plan;
    let panoramaUrl="";
    if(saved?.panoramaPath&&fs.existsSync(saved.panoramaPath))panoramaUrl=server.registerMedia(saved.panoramaPath,true);
    const allAnnotations=Array.isArray(saved?.annotations)?saved.annotations:[];
    const stages=(saved?.stages||[]).map(stage=>({
      ...stage,
      annotations:allAnnotations.filter(a=>(stage.annotationIds||[]).includes(a.annotationId)||a.stageId===stage.stageId),
      imageUrl:stage.imagePath&&fs.existsSync(stage.imagePath)?server.registerMedia(stage.imagePath,true):"",
      backgroundUrl:stage.backgroundPath&&fs.existsSync(stage.backgroundPath)?server.registerMedia(stage.backgroundPath,true):""
    }));
    server.send({type:"treatment_plan",plan:{...saved,stages},panoramaUrl,patient:state.patient});
    setDisplay({mode:"treatment_plan",imageVisible:false});disableViewerKeys();return true;
  });
  ipcMain.handle("plan:navigate",(_e,action)=>{server.send({type:"plan_navigate",action:String(action||"")},false);return true;});
...
if(!gotSingleInstanceLock){
  ...
  app.on("second-instance",(_event,argv)=>{
    ...
    const silent=url&&protocolPayload(url).action==="select_patient";
    if(url){
...
app.whenReady().then(async()=>{
  ...
  archive=new PatientArchive({app,settings,onState:s=>{state.patient=s;emit();},onNotice:notice});
