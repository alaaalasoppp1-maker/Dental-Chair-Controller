...
function buildAppointmentVevent(data){
  ...
  const appointment=new Date(y,mo-1,d,12,0,0,0);
  ...
  // الحدث المقروء من QR هو حدث كامل في اليوم السابق للموعد.
  // DTEND في أحداث اليوم الكامل حصري، لذلك يجب أن يكون اليوم التالي لبداية الحدث.
  const reminder=new Date(appointment);reminder.setDate(reminder.getDate()-1);
  const exclusiveEnd=new Date(reminder);exclusiveEnd.setDate(exclusiveEnd.getDate()+1);
  ...
  return [
    ...
    `DTSTART;VALUE=DATE:${localIcsDay(reminder)}`,
    `DTEND;VALUE=DATE:${localIcsDay(exclusiveEnd)}`,
