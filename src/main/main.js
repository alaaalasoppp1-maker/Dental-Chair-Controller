...
const {contextFromCommand,normalizeAssistantSession,CONTRACT_NAME,TRANSPORT_PROTOCOL}=require("../shared/clinical-contract");
...
function selectClinicalPatient(payload={}){
  const context=contextFromCommand(payload);state.patient=archive?.select(context.patient)||{selected:false};
...
function saveAssistantSession(payload={}){
  const normalized=normalizeAssistantSession(payload,state.clinical?.context);const active=requireMatchingClinicalPatient(normalized.patientId,normalized.fileNo),saved=archive.saveAssistantSession(normalized);
...
function closeAssistantPlan(payload={}){
  ...
  const event=pushClinicalEvent("assistant_plan_closed",{patientId:active.patientId,fileNo:active.fileNo,planId,payload:{doctorName:String(payload.doctorName||context.patient?.doctorName||""),reason:String(payload.reason||"closed_by_doctor")}});
...
app.whenReady().then(async()=>{
  ...
  server=new ChairServer({
    ...
    getHelloPayload:displayConfig,onCommand:handleCommand,onAssistantSession:saveAssistantSession,onAssistantPlanClosed:closeAssistantPlan,onAssistantMedia:saveAssistantMedia,getClinicalEvents
  });
