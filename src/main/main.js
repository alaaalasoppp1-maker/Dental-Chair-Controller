...
function buildAppointmentVevent(data){
  ...
  const appointment=new Date(y,mo-1,d,h||0,mi||0,0);
  if(Number.isNaN(appointment.getTime()))throw new Error("تاريخ الموعد غير صالح");
  // الحدث نفسه يُنشأ في اليوم السابق، لا في يوم الموعد مع منبّه فقط.
  const reminder=new Date(appointment);
  reminder.setDate(reminder.getDate()-1);
  const end=new Date(reminder);
  ...
  return [
    ...
    "METHOD:PUBLISH",
    "BEGIN:VEVENT",
    `UID:dtdc-${Date.now()}-${Math.random().toString(16).slice(2)}@dtdc.local`,
