...
class ChairServer {
  ...
  diagnostics(){
    ...
    return {...base,serverRunning:Boolean(this.server?.listening),startedAt:this.startedAt,lastConnectedAt:this.lastConnectedAt,lastMessageAt:this.lastMessageAt,lastCommandAt:this.lastCommandAt,lastAckAt:this.lastAckAt,lastAckLatencyMs:this.lastAckLatencyMs,lastFailedCommand:this.lastFailedCommand,protocol:3,controllerVersion:"2.8.2",log:[...this.diagnosticLog].reverse()};
  }
  ...
  onMessage(raw){
    try{
      ...
      if(message.type==="ack"&&message.messageId){
        ...
        if(pending){this.lastAckLatencyMs=Date.now()-pending.sentAt;this.lastAckAt=Date.now();this.logDiagnostic("success","ack",{command:pending.payload?.type||"",latencyMs:this.lastAckLatencyMs});}
        this.pending.delete(String(message.messageId));
      }
