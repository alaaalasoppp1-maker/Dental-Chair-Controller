...
class ChairServer {
  ...
  state() {
    ...
    const assistants=this.assistants(),displayClients=this.roleCount(CLIENT_ROLES.DISPLAY),assistantClients=Math.max(this.roleCount(CLIENT_ROLES.ASSISTANT),assistants.filter(item=>item.online).length),unknownClients=this.roleCount(CLIENT_ROLES.UNKNOWN);
    return {
      ...
      ,assistants,selectedAssistantId:this.selectedAssistantId
    };
  }
  ...
  async start() {
    ...
    app.use((req,res,next)=>{
      ...
      res.setHeader("Access-Control-Allow-Headers","Content-Type, X-DTDC-Device-Id, X-DTDC-Display, X-DTDC-File-Name, X-DTDC-Mime-Type, X-DTDC-Media-Kind, X-DTDC-Patient-Id, X-DTDC-Plan-Id, X-DTDC-Session-Id");
    ...
    this.wss.on("connection",socket=>{
      ...
      socket.send(JSON.stringify({type:"hello",protocol:TRANSPORT_PROTOCOL,controllerVersion:"3.1.0",contract:"dtdc-clinical-link-v1",sessionId:this.sessionId,...this.getHelloPayload()}));
      this.emit();
    });
