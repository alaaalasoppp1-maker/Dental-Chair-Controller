...
function contextFromCommand(payload={}){
  const patient=normalizePatient(payload),plans=cleanArray(payload.plans).filter(plan=>!["done","completed","closed","archived","deleted"].includes(text(plan.status).toLowerCase())).map(normalizePlan);
...
function normalizeAssistantStage(value={},context=null){
  const payload=cleanObject(value),patient=cleanObject(payload.patient),activePatient=cleanObject(context?.patient);
  const patientId=text(payload.patientId||patient.patientId||activePatient.patientId),fileNo=text(payload.fileNo||patient.fileNo||activePatient.fileNo);
  const planId=text(payload.planId||payload.activePlanId),stageId=text(payload.stageId||payload.currentStageId);
  if(!patientId&&!fileNo)throw new Error("patientId مطلوب");
  if(!planId)throw new Error("planId مطلوب");
  if(!stageId)throw new Error("stageId مطلوب");
  const status=["pending","active","completed","ready_to_close"].includes(text(payload.status))?text(payload.status):"completed";
  return{
    schema:"dtdc-assistant-stage-v1",contract:CONTRACT_NAME,contractVersion:CONTRACT_VERSION,
    eventId:text(payload.eventId)||`stage-${Date.now()}`,patientId,fileNo,planId,stageId,
    serviceId:text(payload.serviceId),status,completed:Boolean(payload.completed??status==="completed"),
    completedAt:text(payload.completedAt)||(status==="completed"?new Date().toISOString():""),
    sessionId:text(payload.sessionId||activePatient.sessionId),doctorName:text(payload.doctorName||activePatient.doctorName),
    summary:text(payload.summary||payload.note),progress:Math.max(0,Math.min(100,Number(payload.progress||0)||0))
  };
}

module.exports={CONTRACT_NAME,CONTRACT_VERSION,TRANSPORT_PROTOCOL,CLIENT_ROLES,SERVICE_IDS,normalizePatient,normalizePlan,contextFromCommand,normalizeRole,normalizeAssistantSession,normalizeAssistantStage};
